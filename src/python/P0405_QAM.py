...
qpsk_iq = [x + y for x in qpsk_v for y in qpsk_v*1j] # 長さ4のリスト
...
plt.grid()
for i in range(len(qpsk_iq)): #0から3
    x = i%2 # 0101
    y = i//2 # 2,3で1　#text関数(x,y,表示したい文字列)
    plt.text(qpsk_x[i], qpsk_y[i]-0.1, format(GrayCode[y], "01b")+format(GrayCode[x], "01b"),
     fontsize=8, horizontalalignment="center")
# -0.1はそのすぐ下に表示するため yが左側のビット '0表示したい2進の桁数b'

_16qam_v = np.array([-3.0, -1.0, 1.0, 3.0])*(1.0/3.0) #端が１となるように
