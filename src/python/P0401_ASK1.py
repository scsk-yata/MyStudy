...
duration = 10.0e-3                 # 0.010まで

xs = np.arange(0, duration, 1/sample_rate)             # 1000個の値
...
bs_resampled = np.array([bs[i//(len(ys)//len(bs))] 
     for i in range(len(xs))])     # アップサンプリング 20nitを1000個の値へ
ms = ys * (bs_resampled*0.5+0.5)                       # ASK変調

fig = plt.figure()
ax1 = fig.add_subplot(2, 1, 1)     # 2行1列のうちの上側
