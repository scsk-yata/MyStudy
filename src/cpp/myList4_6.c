...
void _qpskMod(SEQ_SIG s, SEQ_DATA data)
{
	unsigned int i, co;
	double c = 1 / sqrt(2.0);
	if (s.len * 2 != data.len)
	{
		printf("Error: _qpskMod, 長さが一致しません．\n");
		exit(1);
	}
	else
	{
		...
		{
			co = 2 * i;
			(s.sig + i)->I = c * ((double)*(data.dat + co) - 0.5) * 2.0;
			(s.sig + i)->Q = c * ((double)*(data.dat + co + 1) - 0.5) * 2.0;
		}
	}
}
void _qpskDem(SEQ_DATA rData, SEQ_SIG rs)
{
	unsigned int i, co;
...
int main(void)
{
	...
	for (i = 0; i < SNR_STEP; i++)
	{
		srand(time(NULL));
		_randData(tx.data);
		_qpskMod(tx.symbol, tx.data);
		_awgn(rx.noise, _SNRdB2noisePower(SNRdB[i]));
		_vectorSum(rx.symbol, tx.symbol, rx.noise);
		_qpskDem(rx.data, rx.symbol);
