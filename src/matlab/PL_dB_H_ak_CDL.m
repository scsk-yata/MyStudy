...
function [PL_dB,H_ak] = PL_dB_H_ak_CDL(d_tu,hor_tu)
...
if P_LOS > rand
    ...
    %PL_dB = 28.0 + ple_LOS*10*log10(d_tu) + 20*log10(fc/(10^9));
...
cdl.TransmitAntennaArray.Orientation        = [90-hor_tu; 0; 0];
...
%% NLOSの場�?
    ...
    %PL_dB = 22.7 + ple_NLOS*10*log10(d_tu) + 26*log10(fc/(10^9));      % 距離はm単位で良�?
    PL_dB = 22.4 + 35.3*log10(sqrt(d_tu^2+10^2)) + 21.3*log10(fc/(10^9));
% パラメータ設�?
DelayProfile = 'CDL-A';
...
cdl.DelaySpread = 300e-9;   % 300 ns 100mにな�?
...
H_Power = mean(mean(abs(H_k_raw).^2));       % 全アン�?ナ�?�?, 全サブキャリアの場合�?�平�?電�?

H_ak = H_k_raw / sqrt(H_Power);    % チャネル応答�?�絶対値の2乗�?�全平�?�?1にした�?
