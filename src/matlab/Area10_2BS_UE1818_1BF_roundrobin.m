%% ２基地局平行設置 20-1818-1 allocationで一様にユーザを分布させた場合
clear
clc
rng('shuffle');
...
carf = ones(N_RB,1) * [1 12 23 34 45 56 67 78 89 100 2 13 24 35 46 57 68 79 90 91 3 14 25 36 47 58 69 80 81 92 4 15 26 37 48 59 70 71 82 93 5 16 27 38 49 60 61 72 83 94 6 17 28 39 50 51 62 73 84 95 7 18 29 40 41 52 63 74 85 96 8 19 30 31 42 53 64 75 86 97 9 20 21 32 43 54 65 76 87 98 10 11 22 33 44 55 66 77 88 99];
carf = repmat(carf,1,3);
...
desire_uk = zeros(Nu_all,N_subcar);
...
AC1 = combnk(1:N_area/2,Nu_1RB);
AC2 = combnk(N_area/2+1:N_area,Nu_1RB);
...
for u = 1:Nu_TP1
    hor_tu(1,u) = d_hor/2+(180-d_hor/2)*rand;
end

for u = Nu_TP1+1:Nu_all
    hor_tu(2,u) = d_hor/2+(180-d_hor/2)*rand;
end
...
%{
[~,c1rfrr] = sort(hor_1all);
c1rfrr = repmat(c1rfrr,N_RB,Nu_TP2);

[~,c2rfrr] = sort(hor_2all);
c2rfrr = repmat(c2rfrr,N_RB,Nu_TP1);
%}

% 基地局1について
for ai = 1:N_area/2
    au = find( ( hor_1area*(ai-1) < hor_1all ) & ( hor_1all <= hor_1area*ai ) );
...
% 基地局２について
for ai = (N_area/2)+1:N_area
    au = find( ( hor_1area*(ai-(N_area/2)-1) < hor_2all ) & ( hor_2all <= hor_1area*(ai-(N_area/2)) ) );
    ...
    elseif length(au) == 1
        ...
        mua2(count_mua2) = ai;
        area_user(ai,1:length(au)) = au+Nu_TP1*ones(1,length(au));
        u2a(au+Nu_TP1) = ai;
        N_aut(ai,trial) = length(au);
...
right_user2 = right_user2 + Nu_TP1*ones(1,length(right_user2));
...
a133 = a_128_1(hor_tu(1,33),90);
a233 = a_128_1(hor_tu(2,33),90);
a134 = a_128_1(hor_tu(1,34),90);
a234 = a_128_1(hor_tu(2,34),90);
a135 = a_128_1(hor_tu(1,35),90);
a235 = a_128_1(hor_tu(2,35),90);
a136 = a_128_1(hor_tu(1,36),90);
a236 = a_128_1(hor_tu(2,36),90);
...
%% 希望信号電力と干渉信号電力
    ...
    R_signal133 = sum( (reshape(H_uak(33,:,k),N_hor*N_ver,1).*a133*ones(1,N_beam)) .*W .*Amp_ak ,1 );
    R_signal233 = sum( (reshape(H_uak(33,:,k),N_hor*N_ver,1).*a233*ones(1,N_beam)) .*W .*Amp_ak ,1 );
    R_signal134 = sum( (reshape(H_uak(34,:,k),N_hor*N_ver,1).*a134*ones(1,N_beam)) .*W .*Amp_ak ,1 );
    R_signal234 = sum( (reshape(H_uak(34,:,k),N_hor*N_ver,1).*a234*ones(1,N_beam)) .*W .*Amp_ak ,1 );
    R_signal135 = sum( (reshape(H_uak(35,:,k),N_hor*N_ver,1).*a135*ones(1,N_beam)) .*W .*Amp_ak ,1 );
    R_signal235 = sum( (reshape(H_uak(35,:,k),N_hor*N_ver,1).*a235*ones(1,N_beam)) .*W .*Amp_ak ,1 );
    R_signal136 = sum( (reshape(H_uak(36,:,k),N_hor*N_ver,1).*a136*ones(1,N_beam)) .*W .*Amp_ak ,1 );
    R_signal236 = sum( (reshape(H_uak(36,:,k),N_hor*N_ver,1).*a236*ones(1,N_beam)) .*W .*Amp_ak ,1 );
    ...
    P_tul(1,33,:) = ( R_signal133.*conj(R_signal133) ) * 10^(-PL_dB_tu(1,33)/10);
    P_tul(2,33,:) = ( R_signal233.*conj(R_signal233) ) * 10^(-PL_dB_tu(2,33)/10);
    P_tul(1,34,:) = ( R_signal134.*conj(R_signal134) ) * 10^(-PL_dB_tu(1,34)/10);
    P_tul(2,34,:) = ( R_signal234.*conj(R_signal234) ) * 10^(-PL_dB_tu(2,34)/10);
    P_tul(1,35,:) = ( R_signal135.*conj(R_signal135) ) * 10^(-PL_dB_tu(1,35)/10);
    P_tul(2,35,:) = ( R_signal235.*conj(R_signal235) ) * 10^(-PL_dB_tu(2,35)/10);
    P_tul(1,36,:) = ( R_signal136.*conj(R_signal136) ) * 10^(-PL_dB_tu(1,36)/10);
    P_tul(2,36,:) = ( R_signal236.*conj(R_signal236) ) * 10^(-PL_dB_tu(2,36)/10);
    ...
    b(17,k) = find( P_tul(1,17,:) == max(P_tul(1,17,:)) ,1 );
    b(18,k) = find( P_tul(1,18,:) == max(P_tul(1,18,:)) ,1 );
    ...
    b(33,k) = find( P_tul(2,33,:) == max(P_tul(2,33,:)) ,1 );
    b(34,k) = find( P_tul(2,34,:) == max(P_tul(2,34,:)) ,1 );
    b(35,k) = find( P_tul(2,35,:) == max(P_tul(2,35,:)) ,1 );
    b(36,k) = find( P_tul(2,36,:) == max(P_tul(2,36,:)) ,1 );
...
log2( 1 + P_tul(1,UC1(c1rf(r,f),1),b(UC1(c1rf(r,f),1)))/(P_tul(2,UC1(c1rf(r,f),1),b(UC2(c2rf(r,f),1))) + Npw) );
          
cumulative_throughput_ut(UC2(c2rf(r,f),1),trial) = cumulative_throughput_ut(UC2(c2rf(r,f),1),trial) + ...
log2( 1 + P_tul(2,UC2(c2rf(r,f),1),b(UC2(c2rf(r,f),1)))/(P_tul(1,UC2(c2rf(r,f),1),b(UC1(c1rf(r,f),1))) + Npw) );
...
log2( 1 + P_tul(1,UC1(c1rfco(r,f),1),b(UC1(c1rfco(r,f),1)))/(P_tul(2,UC1(c1rfco(r,f),1),b(UC2co(c2rfco(r,f),1))) + Npw) );

cumulative_throughput_utc(UC2co(c2rfco(r,f),1),trial) = cumulative_throughput_utc(UC2co(c2rfco(r,f),1),trial) + ...
log2( 1 + P_tul(2,UC2co(c2rfco(r,f),1),b(UC2co(c2rfco(r,f),1)))/(P_tul(1,UC2co(c2rfco(r,f),1),b(UC1(c1rfco(r,f),1))) + Npw) );
...
log2( 1 + P_tul(1,UC1ce(c1rfce(r,f),1),b(UC1ce(c1rfce(r,f),1)))/(P_tul(2,UC1ce(c1rfce(r,f),1),b(UC2(c2rfce(r,f),1))) + Npw) );
        
cumulative_throughput_utc(UC2(c2rfce(r,f),1),trial) = cumulative_throughput_utc(UC2(c2rfce(r,f),1),trial) + ...
log2( 1 + P_tul(2,UC2(c2rfce(r,f),1),b(UC2(c2rfce(r,f),1)))/(P_tul(1,UC2(c2rfce(r,f),1),b(UC1ce(c1rfce(r,f),1))) + Npw) );
...
log2( 1 + P_tul(1,user_trf(1,r,f),b(user_trf(1,r,f)))/(P_tul(2,user_trf(1,r,f),b(user_trf(2,r,f))) + Npw) );
          
cumulative_throughput_utp(user_trf(2,r,f),trial) = cumulative_throughput_utp(user_trf(2,r,f),trial) + ...
log2( 1 + P_tul(2,user_trf(2,r,f),b(user_trf(2,r,f)))/(P_tul(1,user_trf(2,r,f),b(user_trf(1,r,f))) + Npw) );
...
for f = 1:N_subframe
    for r = 1:N_RB
cumulative_throughput_utp(UC1(c1rfrr(r,f)),trial) = cumulative_throughput_utp(UC1(c1rfrr(r,f)),trial) + ...
log2( 1 + P_tul(1,UC1(c1rfrr(r,f)),b(UC1(c1rfrr(r,f))))/(P_tul(2,UC1(c1rfrr(r,f)),b(UC2(c2rfrr(r,f)))) + Npw) );
          
cumulative_throughput_utp(UC2(c2rfrr(r,f)),trial) = cumulative_throughput_utp(UC2(c2rfrr(r,f)),trial) + ...
log2( 1 + P_tul(2,UC2(c2rfrr(r,f)),b(UC2(c2rfrr(r,f))))/(P_tul(1,UC2(c2rfrr(r,f)),b(UC1(c1rfrr(r,f)))) + Npw) );


cumulative_throughput_atp(u2a(UC1(c1rfrr(r,f))),trial) = cumulative_throughput_atp(u2a(UC1(c1rfrr(r,f))),trial) + ...
log2( 1 + P_tul(1,UC1(c1rfrr(r,f)),b(UC1(c1rfrr(r,f))))/(P_tul(2,UC1(c1rfrr(r,f)),b(UC2(c2rfrr(r,f)))) + Npw) );
          
cumulative_throughput_atp(u2a(UC2(c2rfrr(r,f))),trial) = cumulative_throughput_atp(u2a(UC2(c2rfrr(r,f))),trial) + ...
log2( 1 + P_tul(2,UC2(c2rfrr(r,f)),b(UC2(c2rfrr(r,f))))/(P_tul(1,UC2(c2rfrr(r,f)),b(UC1(c1rfrr(r,f)))) + Npw) );


%% 実際の値
        [throughput_sc_1, throughput_sc_2] = calculate_throughput_2user(power_uuk_all, noise_variance_dBm, r, UC1(c1rfrr(r,f)),UC2(c2rfrr(r,f)));
        
        throughput_sc = zeros(1, Nu_1RB*2);   % 瞬時スループットに相当する
        
        throughput_sc(1) = sum(throughput_sc_1(:))/12; % subcarrier数で正規化
        cumulative_throughput_pro(UC1(c1rfrr(r,f)), f) = cumulative_throughput_pro(UC1(c1rfrr(r,f)), f) + throughput_sc(1);
        throughput_sc(2) = sum(throughput_sc_2(:))/12; % subcarrier数で正規化
        cumulative_throughput_pro(UC2(c2rfrr(r,f)), f) = cumulative_throughput_pro(UC2(c2rfrr(r,f)), f) + throughput_sc(2);
...
legend({'セル内干渉のみを考慮してPFした場合','従来法の場合','ラウンドロビンの場合'},'Location','southeast','FontSize',12)
