...
for subframe = 1:N_subframe
    ...
    if AC1(carf1(rb,subframe),2) - AC1(carf1(rb,subframe),1) == 1
        ...
        area1 = AC1(carf1(rb,subframe),2);
        nnz1 = nnz(area_user(area1,:));
        
        if nnz1 == 0
            area1 = AC1(carf1(rb,subframe),2)-1;
            nnz1 = nnz(area_user(area1,:));
            if nnz1 == 0
                [~,mi] = min(abs( mua1-area1 ));
                area1 = mua1(mi);
        ...
        candidate1 = 1:Nu_TP1;
        if nnz2 > 0
            candidate1(ismember(candidate1,area_user(area2,1:nnz2))) = [];
        end
        power_uuk1p = power_uuk1(candidate1,candidate1,:);
        uc1 = zeros(nnz1*(Nu_TP1-nnz1-nnz2), Nu_1RB);
        combination = zeros(nnz1*(Nu_TP1-nnz1-nnz2), Nu_1RB);
        
        candidate_ = candidate1;
        candidate_(ismember(candidate_,area_user(area1,1:nnz1))) = [];
        
for c1 = 1:nnz1
    for c2 = 1:(Nu_TP1-nnz1-nnz2)
        uc1((c1-1)*(Nu_TP1-nnz1-nnz2)+c2,:) = [area_user(area1,c1),candidate_(c2)];
        combination((c1-1)*(Nu_TP1-nnz1-nnz2)+c2,:) = [c1,nnz1+c2];
    end
end
    ...
    %% 基地局2について
    if AC2(carf2(rb,subframe),2) - AC2(carf2(rb,subframe),1) == 1
        ...
        candidate2 = Nu_TP1+1:Nu_all;
        if nnz4 > 0
            candidate2(ismember(candidate2,area_user(area4,1:nnz4))) = [];
        end
        power_uuk2p = power_uuk_all(candidate2,candidate2,:);
        uc2 = zeros(nnz3*(Nu_TP2-nnz3-nnz4), Nu_1RB);
        combination = zeros(nnz3*(Nu_TP2-nnz3-nnz4), Nu_1RB);
        
        candidate_ = candidate2;
        candidate_(ismember(candidate_,area_user(area3,1:nnz3))) = [];
        
for c1 = 1:nnz3
    for c2 = 1:(Nu_TP1-nnz3-nnz4)
        uc2((c1-1)*(Nu_TP1-nnz3-nnz4)+c2,:) = [area_user(area3,c1),candidate_(c2)];
        combination((c1-1)*(Nu_TP1-nnz3-nnz4)+c2,:) = [c1,nnz3+c2];
    end
end
