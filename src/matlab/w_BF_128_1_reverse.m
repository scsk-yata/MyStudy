...
function W = w_BF_128_1_reverse(hor_angle,ver_angle)
...
for nx = 1:N_hor
    for nz = 1:N_ver
        ...
        W((nx-1)*N_ver+nz,1) = exp(-1j*2*pi*(d/lambda)*((N_hor-nx+1)*cosd(hor_angle)*sind(ver_angle)+(N_ver-nz+1)*cosd(ver_angle)));
