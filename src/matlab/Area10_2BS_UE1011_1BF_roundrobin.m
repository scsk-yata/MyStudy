%% ２基地局平行設置 20-21-1 allocationで一様にユーザを分布させた場合　単純ラウンドロビン+FB
clear
clc
rng('shuffle');
...
[~,c1rr] = sort(hor_1all);
c1rfrr = repmat(c1rr,N_RB,Nu_TP2);
c1rfrr = repmat(c1rfrr,1,3);

[~,c2rr] = sort(hor_2all);
c2rfrr = repmat(c2rr,N_RB,Nu_TP1);
c2rfrr = repmat(c2rfrr,1,3);
...
for subframe = 1:N_subframe
    ...
    for rb = 1:N_RB
        ...
        if isempty(left_user1)
            for rb = 1:N_RB
        %c1rfce(rb, subframe) = round(0.5+9.9999*rand);
        c1rfce(rb, subframe) = 0;
            end
            %%
        else
...
for subframe = 1:N_subframe
    if rem(subframe,2) == 1
        ...
        if isempty(right_user2)
             for rb = 1:N_RB
        %c2rfco(rb, subframe) = round(0.5+9.9999*rand);
        c2rfco(rb, subframe) = 0;
             end
...
throughput_rf1 = zeros(N_RB,N_subframe);
throughput_rf2 = zeros(N_RB,N_subframe);

Nprf = round((Nu_TP1+Nu_TP2)/4);
pfrf1 = zeros(N_RB,Nu_TP1*Nprf);
pfrf2 = zeros(N_RB,Nu_TP2*Nprf);
...
throughput_rf1(r,f) = log2( 1 + P_tul(1,UC1(c1rfrr(r,f)),b(UC1(c1rfrr(r,f))))/(P_tul(2,UC1(c1rfrr(r,f)),b(UC2(c2rfrr(r,f)))) + Npw) );
throughput_rf2(r,f) = log2( 1 + P_tul(2,UC2(c2rfrr(r,f)),b(UC2(c2rfrr(r,f))))/(P_tul(1,UC2(c2rfrr(r,f)),b(UC1(c1rfrr(r,f)))) + Npw) );
...
%% 実際の値
        ...
        cumulative_throughput_all(UC1(c1rfrr(r,f))) = cumulative_throughput_all(UC1(c1rfrr(r,f))) + throughput_sc(1);
        cumulative_throughput_all(UC2(c2rfrr(r,f))) = cumulative_throughput_all(UC2(c2rfrr(r,f))) + throughput_sc(2);
...
%% フィードバックによってPFするフレームの決定
for r = 1:N_RB
    for u = 1:Nu_TP1
        uhi = find(c1rr == u);
        throughput_rfb1 = throughput_rf1(r, [uhi:Nu_TP1:uhi+Nu_TP1*(Nu_TP2-1)] );
        [~,mi1] = mink(throughput_rfb1,Nprf);
        pfrf1(r,(u-1)*Nprf+1:u*Nprf) = [uhi + (mi1-1)*Nu_TP1];
    end
        pfrf1(r,:) = sort(pfrf1(r,:));
end

pfrf1 = horzcat( (pfrf1(:,:)+(Nu_TP1*Nu_TP2)) , (pfrf1(:,:)+(Nu_TP1*Nu_TP2*2)) );


for r = 1:N_RB
    for u = 1:Nu_TP2
        uhi = find(c2rr == u);
        throughput_rfb2 = throughput_rf2(r, [uhi:Nu_TP2:uhi+Nu_TP2*(Nu_TP1-1)] );
        [~,mi2] = mink(throughput_rfb2,Nprf);
        pfrf2(r,(u-1)*Nprf+1:u*Nprf) = [uhi + (mi2-1)*Nu_TP2];
    end
        pfrf2(r,:) = sort(pfrf2(r,:));
end

pfrf2 = horzcat( (pfrf2(:,:)+(Nu_TP1*Nu_TP2)) , (pfrf2(:,:)+(Nu_TP1*Nu_TP2*2)) );




%% 基地局1について
for subframe = (Nu_TP1*Nu_TP2)+1:N_subframe
    
    for rb = 1:N_RB
        
        tp = 1;
        
        if ismember(subframe,pfrf1(r,:))      % PFフレームの場合
            
            combination = combnk(1:Nu_TP1,Nu_1RB);
            N_combination = numel(combination(:,1));
            
        cumulative_throughput_temp = cumulative_throughput_all(1:Nu_TP1);
        zero_judge = all(cumulative_throughput_all(1:Nu_TP1));
        zero_user_num = 0;
        ...
        for UE_comb_index = 1:N_combinations_temp
            ...
            [throughput_est_sc_1] = calculate_throughput_1user(power_uuk1, noise_variance_dBm, rb, c1rfp(rb, subframe), combination_temp );
            ...
            elseif zero_judge == 1
                normalization_value = N_RB * (subframe - 1);
                for j = 1:Nu_1RB
                    ratio_now = ( throughput_EST_sc(j)) / (( cumulative_throughput_all( combination_temp(c1rfp(rb, subframe), j) ))/normalization_value);
                    ratio = ratio * ( 1 + ratio_now );
                end
            end
            ...
            user_trf(tp,rb,subframe) = c1rfp(rb, subframe);
            ...
            c1rfp(rb, subframe) = c1rfrr(rb,subframe);
            user_trf(tp,rb,subframe) = c1rfp(rb, subframe);
            count_user_allocated(user_trf(tp,rb,subframe)) = count_user_allocated(user_trf(tp,rb,subframe)) + 1;
...
%% 基地局2について
        ...
        for UE_comb_index = 1:N_combinations_temp
            ...
            [throughput_est_sc_1] = calculate_throughput_1user(power_uuk2, noise_variance_dBm, rb, c2rfp(rb, subframe), combination_temp );
            ...
            elseif zero_judge == 1
                normalization_value = N_RB * (subframe - 1);
                for j = 1:Nu_1RB
                    ratio_now = ( throughput_EST_sc(j)) / (( cumulative_throughput_all( combination_temp(c2rfp(rb, subframe), j) ))/normalization_value);
                    ratio = ratio * ( 1 + ratio_now );
                end
            end
            ...
            user_trf(tp,rb,subframe) = c2rfp(rb, subframe);
            count_user_allocated(user_trf(tp,rb,subframe)) = count_user_allocated(user_trf(tp,rb,subframe)) + 1;
            ...
            c2rfp(rb, subframe) = c2rfrr(rb,subframe);
            user_trf(tp,rb,subframe) = c2rfp(rb, subframe);
            count_user_allocated(user_trf(tp,rb,subframe)) = count_user_allocated(user_trf(tp,rb,subframe)) + 1;
...
%%
for f = (Nu_TP1*Nu_TP2)+1:N_subframe
    for r = 1:N_RB
cumulative_throughput_utp(UC1(c1rfp(r,f)),trial) = cumulative_throughput_utp(UC1(c1rfp(r,f)),trial) + ...
log2( 1 + P_tul(1,UC1(c1rfp(r,f)),b(UC1(c1rfp(r,f))))/(P_tul(2,UC1(c1rfp(r,f)),b(UC2(c2rfp(r,f)))) + Npw) );
          
cumulative_throughput_utp(UC2(c2rfp(r,f)),trial) = cumulative_throughput_utp(UC2(c2rfp(r,f)),trial) + ...
log2( 1 + P_tul(2,UC2(c2rfp(r,f)),b(UC2(c2rfp(r,f))))/(P_tul(1,UC2(c2rfp(r,f)),b(UC1(c1rfp(r,f)))) + Npw) );


cumulative_throughput_atp(u2a(UC1(c1rfp(r,f))),trial) = cumulative_throughput_atp(u2a(UC1(c1rfp(r,f))),trial) + ...
log2( 1 + P_tul(1,UC1(c1rfp(r,f)),b(UC1(c1rfp(r,f))))/(P_tul(2,UC1(c1rfp(r,f)),b(UC2(c2rfp(r,f)))) + Npw) );
          
cumulative_throughput_atp(u2a(UC2(c2rfp(r,f))),trial) = cumulative_throughput_atp(u2a(UC2(c2rfp(r,f))),trial) + ...
log2( 1 + P_tul(2,UC2(c2rfp(r,f)),b(UC2(c2rfp(r,f))))/(P_tul(1,UC2(c2rfp(r,f)),b(UC1(c1rfp(r,f)))) + Npw) );

            
%% 実際の値
        [throughput_sc_1, throughput_sc_2] = calculate_throughput_2user(power_uuk_all, noise_variance_dBm, r, UC1(c1rfp(r,f)),UC2(c2rfp(r,f)));
        
        throughput_sc = zeros(1, Nu_1RB*2);   % 瞬時スループットに相当する
        
        throughput_sc(1) = sum(throughput_sc_1(:))/12; % subcarrier数で正規化
        cumulative_throughput_pro(UC1(c1rfp(r,f)), f) = cumulative_throughput_pro(UC1(c1rfp(r,f)), f) + throughput_sc(1);
        throughput_sc(2) = sum(throughput_sc_2(:))/12; % subcarrier数で正規化
        cumulative_throughput_pro(UC2(c2rfp(r,f)), f) = cumulative_throughput_pro(UC2(c2rfp(r,f)), f) + throughput_sc(2);
        
        
        cumulative_throughput_all(UC1(c1rfp(r,f))) = cumulative_throughput_all(UC1(c1rfp(r,f))) + throughput_sc(1);
        cumulative_throughput_all(UC2(c2rfp(r,f))) = cumulative_throughput_all(UC2(c2rfp(r,f))) + throughput_sc(2);
...
save( [file_name,'/cumulative_throughput_',num2str(trial),'trial'] , 'cumulative_throughput', 'cumulative_throughput_con','cumulative_throughput_pro' ,'throughput_rf1' ,'throughput_rf2')
