%% ２基地局平行設置 20-44-1 allocationで一様にユーザを分布させた場合　送信範囲四角形の場合
clear
clc
rng('shuffle');
...
%{
    ...
    b(5,k) = find( P_tul(2,5,:) == max(P_tul(2,5,:)) ,1 );
    b(6,k) = find( P_tul(2,6,:) == max(P_tul(2,6,:)) ,1 );
    b(7,k) = find( P_tul(2,7,:) == max(P_tul(2,7,:)) ,1 );
    b(8,k) = find( P_tul(2,8,:) == max(P_tul(2,8,:)) ,1 );
