%% ２基地局平行設置 12-99-1 allocationで一様にユーザを分布させた場合　送信範囲四角形の場合
clear
clc
rng('shuffle');
...
for u = 1:Nu_TP1
    ...
    if (hor_tu(1,u) <= 45) || (135 <= hor_tu(1,u))
        d_tu(1,u) = d_edge/abs(cosd(hor_tu(1,u)))*rand;
    else
        d_tu(1,u) = d_edge/cosd(abs(90-hor_tu(1,u)))*rand;
    end
...
for u = Nu_TP1+1:Nu_all
    ...
    if (hor_tu(2,u) <= 45) || (135 <= hor_tu(2,u))
        d_tu(2,u) = d_edge/abs(cosd(hor_tu(2,u)))*rand;
    else
        d_tu(2,u) = d_edge/cosd(abs(90-hor_tu(2,u)))*rand;
    end
