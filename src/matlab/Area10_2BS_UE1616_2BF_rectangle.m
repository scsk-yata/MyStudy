%% ２基地局平行設置 20-1616-2 allocationで一様にユーザを分布させた場合　送信範囲四角形の場合
clear
clc
rng('shuffle');
...
area_boundary = [0,atand(4/N_area_1TP),atand(2*4/N_area_1TP),90-atand(2*4/N_area_1TP),90-atand(4/N_area_1TP)...
    ,90,90+atand(4/N_area_1TP),90+atand(2*4/N_area_1TP),180-atand(2*4/N_area_1TP),180-atand(4/N_area_1TP),180];
...
%{
ca1 = [4,13,21,28,34,39,6,15,23,30,5,14,22,29,35];
%ca1 = [4,13,21,28,34,39,6,15,23,30];
carf1 = ones(N_RB,1) * ca1;
carf1 = repmat(carf1,1,30);

ca2 = [4,13,21,28,34,39,6,15,23,30,5,14,22,29,35];
carf2 = ones(N_RB,1) * [ca2,circshift(ca2,1),circshift(ca2,2),circshift(ca2,3),circshift(ca2,4),circshift(ca2,5),circshift(ca2,6),circshift(ca2,7),circshift(ca2,8),circshift(ca2,9),circshift(ca2,10),circshift(ca2,11),circshift(ca2,12),circshift(ca2,13),circshift(ca2,14)];
carf2 = repmat(carf2,1,2);
N_timeslot = length(carf1)*7;
N_subframe = N_timeslot/7;
%}

ca1 = [2,11,19,26,32,37,41,44,8,17];
carf1 = ones(N_RB,1) * ca1;
carf1 = repmat(carf1,1,20);

ca2 = [2,11,19,26,32,37,41,44,8,17];
carf2 = ones(N_RB,1) * [ca2,circshift(ca2,1),circshift(ca2,2),circshift(ca2,3),circshift(ca2,4),circshift(ca2,5),circshift(ca2,6),circshift(ca2,7),circshift(ca2,8),circshift(ca2,9)];
carf2 = repmat(carf2,1,2);
N_timeslot = length(carf1)*7;
N_subframe = N_timeslot/7;
...
user_irf = zeros(N_TP*Nu_1RB,N_RB,N_subframe);
...
for subframe = 1:N_subframe
    ...
    for rb = 1:N_RB
        ...
        candidate1 = [area_user(area1,1:nnz1),area_user(area2,1:nnz2)];
        power_uuk1p = power_uuk1(candidate1,candidate1,:);
        uc1 = zeros(nnz1*nnz2, Nu_1RB);
        combination = zeros(nnz1*nnz2, Nu_1RB);
        
        
for c1 = 1:nnz1
    for c2 = 1:nnz2
        uc1((c1-1)*nnz2+c2,:) = [area_user(area1,c1),area_user(area2,c2)];
        combination((c1-1)*nnz2+c2,:) = [c1,nnz1+c2];
    end
end

        N_combination = numel(combination(:,1));
            
        cumulative_throughput_temp = cumulative_throughput_all(candidate1);
        zero_judge = all(cumulative_throughput_all(candidate1));
        zero_user_num = 0;
        ...
        if zero_judge == 1  % 0UEなし
            ...
            zero_user_num = numel(zero_user_array);
                ...
                count = 0;
                for combination_index = 1:N_combination
                   if sum(ismember(combination_temp(combination_index, :),zero_user_array))==0
                       count = count + 1;
                       removal_line(count) = combination_index;
                   end
                end
        ...
        for UE_comb_index = 1:N_combinations_temp
            ...
            [throughput_est_sc_1, throughput_est_sc_2] = calculate_throughput_PF(power_uuk1p, noise_variance_dBm, rb, c1rfp(rb, subframe), combination_temp );
            ...
            if zero_judge == 0
                ...
                for j = 1:Nu_1RB
                    if ismember(combination_temp(c1rfp(rb, subframe), j),zero_user_array)
                        normalization_value = 1;
                        ratio_now = ( throughput_EST_sc(j)) / (( cumulative_throughput_ones(combination_temp(c1rfp(rb, subframe), j)))/normalization_value);
                    else
                        normalization_value = N_RB * (subframe - 1);
                        ratio_now = ( throughput_EST_sc(j)) / (( cumulative_throughput_all(candidate1(combination_temp(c1rfp(rb, subframe), j))))/normalization_value);
                    end
                    ratio = ratio * ( 1 + ratio_now );
                end
            elseif zero_judge == 1
        ...
        user_irf(1,rb,subframe) = uc1(c1rfp(rb, subframe),1);
        user_irf(2,rb,subframe) = uc1(c1rfp(rb, subframe),2);
    ...
    %% 基地局2について
        ...
        candidate2 = [area_user(area3,1:nnz3),area_user(area4,1:nnz4)];
        power_uuk2p = power_uuk_all(candidate2,candidate2,:);
        uc2 = zeros(nnz3*nnz4, Nu_1RB);
        combination = zeros(nnz3*nnz4, Nu_1RB);

for c1 = 1:nnz3
    for c2 = 1:nnz4
        uc2((c1-1)*nnz4+c2,:) = [area_user(area3,c1),area_user(area4,c2)];
        combination((c1-1)*nnz4+c2,:) = [c1,nnz3+c2];
    end
end

        N_combination = numel(combination(:,1));
            
        cumulative_throughput_temp = cumulative_throughput_all(candidate2);
        zero_judge = all(cumulative_throughput_all(candidate2));
        zero_user_num = 0;
        ...
        for UE_comb_index = 1:N_combinations_temp
            ...
            [throughput_est_sc_1, throughput_est_sc_2] = calculate_throughput_PF(power_uuk2p, noise_variance_dBm, rb, c2rfp(rb, subframe), combination_temp );
            ...
            if zero_judge == 0
                ...
                for j = 1:Nu_1RB
                    if ismember(combination_temp(c2rfp(rb, subframe), j),zero_user_array)
                        normalization_value = 1;
                        ratio_now = ( throughput_EST_sc(j)) / (( cumulative_throughput_ones(combination_temp(c2rfp(rb, subframe), j)))/normalization_value);
                    else
                        normalization_value = N_RB * (subframe - 1);
                        ratio_now = ( throughput_EST_sc(j)) / (( cumulative_throughput_all(candidate2(combination_temp(c2rfp(rb, subframe), j))))/normalization_value);
                    end
                    ratio = ratio * ( 1 + ratio_now );
                end
            elseif zero_judge == 1
        ...
        user_irf(3,rb,subframe) = uc2(c2rfp(rb, subframe),1);
        user_irf(4,rb,subframe) = uc2(c2rfp(rb, subframe),2);
        
        user_allocated = [user_irf(1,rb,subframe),user_irf(2,rb,subframe),user_irf(3,rb,subframe),user_irf(4,rb,subframe)];
        
        
        
        
      %% 実際の値
        [throughput_sc_1,throughput_sc_2,throughput_sc_3,throughput_sc_4] = calculate_throughput_4user(power_uuk_all, noise_variance_dBm, rb, 1,user_allocated);
        
        throughput_sc = zeros(1, Nu_1RB*2);   % 瞬時スループットに相当する
        
        throughput_sc(1) = sum(throughput_sc_1(:))/12; % subcarrier数で正規化
        cumulative_throughput_pro(user_irf(1,rb,subframe), subframe) = cumulative_throughput_pro(user_irf(1,rb,subframe), subframe) + throughput_sc(1);
        throughput_sc(2) = sum(throughput_sc_2(:))/12; % subcarrier数で正規化
        cumulative_throughput_pro(user_irf(2,rb,subframe), subframe) = cumulative_throughput_pro(user_irf(2,rb,subframe), subframe) + throughput_sc(2);
        throughput_sc(3) = sum(throughput_sc_3(:))/12; % subcarrier数で正規化
        cumulative_throughput_pro(user_irf(3,rb,subframe), subframe) = cumulative_throughput_pro(user_irf(3,rb,subframe), subframe) + throughput_sc(3);
        throughput_sc(4) = sum(throughput_sc_4(:))/12; % subcarrier数で正規化
        cumulative_throughput_pro(user_irf(4,rb,subframe), subframe) = cumulative_throughput_pro(user_irf(4,rb,subframe), subframe) + throughput_sc(4);
        
        
        for j = 1:Nu_1RB*2
            cumulative_throughput_all(user_irf(j,rb,subframe)) = cumulative_throughput_all(user_irf(j,rb,subframe)) + throughput_sc(j);
        end
...
for f = 1:N_subframe
    for r = 1:N_RB
cumulative_throughput_utp(user_irf(1,r,f),trial) = cumulative_throughput_utp(user_irf(1,r,f),trial) + ...
log2( 1 + P_tul(1,user_irf(1,r,f),b(user_irf(1,r,f)))/(P_tul(1,user_irf(1,r,f),b(user_irf(2,r,f)))...
+ P_tul(2,user_irf(1,r,f),b(user_irf(3,r,f))) + P_tul(2,user_irf(1,r,f),b(user_irf(4,r,f))) + Npw) );

cumulative_throughput_utp(user_irf(2,r,f),trial) = cumulative_throughput_utp(user_irf(2,r,f),trial) + ...
log2( 1 + P_tul(1,user_irf(2,r,f),b(user_irf(2,r,f)))/(P_tul(1,user_irf(2,r,f),b(user_irf(1,r,f)))...
+ P_tul(2,user_irf(2,r,f),b(user_irf(3,r,f))) + P_tul(2,user_irf(2,r,f),b(user_irf(4,r,f))) + Npw) );

cumulative_throughput_utp(user_irf(3,r,f),trial) = cumulative_throughput_utp(user_irf(3,r,f),trial) + ...
log2( 1 + P_tul(2,user_irf(3,r,f),b(user_irf(3,r,f)))/(P_tul(2,user_irf(3,r,f),b(user_irf(4,r,f)))...
+ P_tul(1,user_irf(3,r,f),b(user_irf(1,r,f))) + P_tul(1,user_irf(3,r,f),b(user_irf(2,r,f))) + Npw) );

cumulative_throughput_utp(user_irf(4,r,f),trial) = cumulative_throughput_utp(user_irf(4,r,f),trial) + ...
log2( 1 + P_tul(2,user_irf(4,r,f),b(user_irf(4,r,f)))/(P_tul(2,user_irf(4,r,f),b(user_irf(3,r,f)))...
+ P_tul(1,user_irf(4,r,f),b(user_irf(1,r,f))) + P_tul(1,user_irf(4,r,f),b(user_irf(2,r,f))) + Npw) );



cumulative_throughput_atp(u2a(user_irf(1,r,f)),trial) = cumulative_throughput_atp(u2a(user_irf(1,r,f)),trial) + ...
log2( 1 + P_tul(1,user_irf(1,r,f),b(user_irf(1,r,f)))/(P_tul(1,user_irf(1,r,f),b(user_irf(2,r,f)))...
+ P_tul(2,user_irf(1,r,f),b(user_irf(3,r,f))) + P_tul(2,user_irf(1,r,f),b(user_irf(4,r,f))) + Npw) );
          
cumulative_throughput_atp(u2a(user_irf(2,r,f)),trial) = cumulative_throughput_atp(u2a(user_irf(2,r,f)),trial) + ...
log2( 1 + P_tul(1,user_irf(2,r,f),b(user_irf(2,r,f)))/(P_tul(1,user_irf(2,r,f),b(user_irf(1,r,f)))...
+ P_tul(2,user_irf(2,r,f),b(user_irf(3,r,f))) + P_tul(2,user_irf(2,r,f),b(user_irf(4,r,f))) + Npw) );

cumulative_throughput_atp(u2a(user_irf(3,r,f)),trial) = cumulative_throughput_atp(u2a(user_irf(3,r,f)),trial) + ...
log2( 1 + P_tul(2,user_irf(3,r,f),b(user_irf(3,r,f)))/(P_tul(2,user_irf(3,r,f),b(user_irf(4,r,f)))...
+ P_tul(1,user_irf(3,r,f),b(user_irf(1,r,f))) + P_tul(1,user_irf(3,r,f),b(user_irf(2,r,f))) + Npw) );

cumulative_throughput_atp(u2a(user_irf(4,r,f)),trial) = cumulative_throughput_atp(u2a(user_irf(4,r,f)),trial) + ...
log2( 1 + P_tul(2,user_irf(4,r,f),b(user_irf(4,r,f)))/(P_tul(2,user_irf(4,r,f),b(user_irf(3,r,f)))...
+ P_tul(1,user_irf(4,r,f),b(user_irf(1,r,f))) + P_tul(1,user_irf(4,r,f),b(user_irf(2,r,f))) + Npw) );
