...
N_TP = 2;
Nu_1RB = 1;
Nu_TP1 = 4;                                    % 左側の基地局の送信範囲ユーザ数
Nu_TP2 = 4;                                    % 右側の基地局の送信範囲ユーザ数
...
for k = 1:N_subcar
    ...
    R_signal15 = sum( (reshape(H_1uak(17,:,k),N_hor*N_ver,1)*ones(1,N_beam)) .*W .*Amp_ak ,1 );
    R_signal25 = sum( (reshape(H_2uak(17,:,k),N_hor*N_ver,1)*ones(1,N_beam)) .*W .*Amp_ak ,1 );
    R_signal16 = sum( (reshape(H_1uak(18,:,k),N_hor*N_ver,1)*ones(1,N_beam)) .*W .*Amp_ak ,1 );
    R_signal26 = sum( (reshape(H_2uak(18,:,k),N_hor*N_ver,1)*ones(1,N_beam)) .*W .*Amp_ak ,1 );
    R_signal17 = sum( (reshape(H_1uak(19,:,k),N_hor*N_ver,1)*ones(1,N_beam)) .*W .*Amp_ak ,1 );
    R_signal27 = sum( (reshape(H_2uak(19,:,k),N_hor*N_ver,1)*ones(1,N_beam)) .*W .*Amp_ak ,1 );
    R_signal18 = sum( (reshape(H_1uak(20,:,k),N_hor*N_ver,1)*ones(1,N_beam)) .*W .*Amp_ak ,1 );
    R_signal28 = sum( (reshape(H_2uak(20,:,k),N_hor*N_ver,1)*ones(1,N_beam)) .*W .*Amp_ak ,1 );
